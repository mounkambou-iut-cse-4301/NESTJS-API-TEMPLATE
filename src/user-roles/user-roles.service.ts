...
//   async list(q: { page:number; pageSize:number; sort?:string; userId?:number; roleId?:number }) {
//     const where: any = {};
...
//     // vérifie user + rôles
//     const user = await this.prisma.utilisateur.findUnique({ where: { id: userId }, select: { id: true } });
//     if (!user) throw new BadRequestException({ message: "Utilisateur invalide.", messageE: "Invalid user." });

//     const roles = await this.prisma.role.findMany({ where: { id: { in: roleIds } }, select: { id: true } });
//     const validIds = new Set(roles.map(r => r.id));
//     const toCreate = roleIds.filter(id => validIds.has(id)).map(roleId => ({ utilisateurId: userId, roleId }));

//     if (!toCreate.length) {
//       throw new BadRequestException({ message: "Aucun rôle valide fourni.", messageE: "No valid role provided." });
//     }

//     await this.prisma.utilisateurRole.createMany({ data: toCreate, skipDuplicates: true });
//     const current = await this.prisma.utilisateurRole.findMany({
//       where: { utilisateurId: userId },
//       select: { role: { select: { id: true, nom: true } } },
//     });
...
//     await this.prisma.utilisateurRole.deleteMany({ where: { utilisateurId: userId, roleId: { in: roleIds } } });
//     const current = await this.prisma.utilisateurRole.findMany({
//       where: { utilisateurId: userId },
//       select: { role: { select: { id: true, nom: true } } },
//     });
